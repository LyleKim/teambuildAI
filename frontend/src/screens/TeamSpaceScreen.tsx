    ...
    >
      ...
      >
        <div className="flex items-center gap-3 mb-5">
          <Avatar
            initial={
              teammate.counterpart.initial ||
              initialOf(teammate.counterpart.name)
            }
            size={40}
          />
          <div>
...
export function TeamSpaceScreen({ hackathonId }: { hackathonId: number }) {
  ...
  const hackathon = useQuery(`hackathon:${hackathonId}`, () =>
    hackathonApi.detail(hackathonId),
  )
  const teammates = useQuery(`teammates:${hackathonId}`, () =>
    coffeechatApi.teammates(hackathonId),
  )
  const todos = useQuery(`todos:${hackathonId}`, () =>
    todoApi.list(hackathonId),
  )
  const manualParticipants = useQuery(
    `manual-participants:${hackathonId}`,
  ...
  const removeTeammate = useMutation((id: number) => coffeechatApi.remove(id), {
    onSuccess: (_void, id) =>
      teammates.setData((prev) => (prev ?? []).filter((t) => t.id !== id)),
  })

  const addTodo = useMutation(
    () => todoApi.create(hackathonId, newTodo.trim()),
    {
      onSuccess: (created) => {
  ...
  const removeTodo = useMutation((id: number) => todoApi.remove(id), {
    onSuccess: (_void, id) =>
      todos.setData((prev) => (prev ?? []).filter((t) => t.id !== id)),
  })
  ...
  const removeManual = useMutation(
    (id: number) => manualParticipantApi.remove(id),
    {
      onSuccess: (_void, id) =>
        manualParticipants.setData((prev) =>
  ...
  const participations = useQuery("me:participations", () =>
    participationApi.mine(),
  )
  const myParticipation =
    (participations.data ?? []).find((p) => p.hackathon.id === hackathonId) ??
    null

  const endProject = useMutation(
    () => participationApi.end(myParticipation!.id),
    {
      onSuccess: (updated) =>
      ...
      {!hackathon.loading && !hackathon.error && hackathon.data && (
        <>
          ...
          <section className="mb-8">
            ...
            <InlineError
              message={
                removeTeammate.error?.message ??
                removeManual.error?.message ??
            ...
            {!teamSectionLoading && teamSectionError && (
              <ErrorState
                error={teamSectionError}
                onRetry={teamSectionRefetch}
              />
            )}
            {!teamSectionLoading &&
              !teamSectionError &&
                  ...
                  entry.kind === "coffeechat" ? (
                    ...
                    >
                      <div className="flex items-start gap-3 mb-3">
                        <button
                          onClick={() =>
                            navigate(routes.member(entry.data.counterpart.id))
                          }
                        >
                          <Avatar
                            initial={
                              entry.data.counterpart.initial ||
                              initialOf(entry.data.counterpart.name)
                            }
                            size={40}
                          />
                        </button>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <Avatar
                          initial={initialOf(entry.data.name)}
                          size={40}
                        />
                        <div className="flex-1 min-w-0">
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" && newTodo.trim())
                    addTodo.mutate(undefined as void)
                }}
            ...
            {myParticipation?.ended_at ? (
              ...
              <button
                onClick={() => {
                  if (
                    window.confirm(
                      "이 프로젝트를 종료할까요? 팀원·할 일 기록은 그대로 남아요.",
                    )
                  ) {
