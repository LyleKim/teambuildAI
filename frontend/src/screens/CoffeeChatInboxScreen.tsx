...
export function CoffeeChatInboxScreen() {
  ...
  const { data, loading, error, refetch, setData } = useQuery<CoffeeChat[]>(
    ...
    () =>
      tab === "received"
        ? coffeechatApi.received(filter)
        : coffeechatApi.sent(filter),
  )
  ...
  const filterOptions =
    tab === "received"
      ...
      : COFFEECHAT_FILTERS
  const busy =
    accept.loading || reject.loading || progress.loading || remove.loading
              ...
              >
                <div className="flex items-start gap-3 mb-3">
                  <button onClick={() => navigate(routes.member(person.id))}>
                    <Avatar
                      initial={person.initial || initialOf(person.name)}
                      size={40}
                    />
                  </button>
