  ...
  },
  {
    ...
    icon: (
      ...
      >
        ...
        <path
          d="M16 13.5c1-.35 2.1-.5 3-.5 2.8 0 5 1.8 5 5"
          strokeOpacity="0.7"
        />
      </svg>
    ),
...
const STEPS = [
  {
    step: "STEP 01",
    title: "해커톤 선택",
    desc: "관심 있는 해커톤을 탐색하고 참가 방식(개인/팀)을 선택해요.",
  },
  {
    step: "STEP 02",
    title: "프로필 · 조건 작성",
    desc: "역할, 스택, 활동 방식 등 희망 조건을 입력하면 AI가 분석을 시작해요.",
  },
  {
    step: "STEP 03",
    title: "추천 & 커피챗",
    desc: "매칭 점수가 높은 상대에게 커피챗을 신청하고 팀을 완성해요.",
  },
]
  ...
  return (
    <div className="min-h-screen bg-white">
      <header className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-8 h-[60px] bg-white/80 backdrop-blur-md border-b border-[#E8EFF8]">
        <div className="flex items-center gap-2.5">
          ...
          <div className="flex flex-col leading-none">
            ...
            <span className="text-[10px] font-medium text-[#4EAAF5] tracking-wide mt-0.5">
              Favorite contact
            </span>
          </div>
        </div>
      ...
      <section
        ...
        style={{
          background:
            "linear-gradient(175deg, #EAF3FD 0%, #F5F9FF 50%, #EDF2FB 100%)",
        }}
      >
        <div
          className="absolute top-20 left-12 w-48 h-48 rounded-full opacity-60 pointer-events-none"
          style={{
            background: "radial-gradient(circle, #A8D8F8 0%, transparent 70%)",
            filter: "blur(32px)",
          }}
        />
        <div
          className="absolute top-32 right-16 w-40 h-40 rounded-full opacity-50 pointer-events-none"
          style={{
            background: "radial-gradient(circle, #7EC8F6 0%, transparent 70%)",
            filter: "blur(28px)",
          }}
        />
        ...
        <div className="relative z-10 max-w-2xl">
          ...
          <h1 className="text-[52px] font-black leading-tight tracking-tight text-gray-900 mb-2">
            AI로 만나는
          </h1>
          ...
          <div className="mt-10 grid grid-cols-4 gap-6">
            {statItems.map(([num, label]) => (
              <div key={label} className="text-center">
                <div className="text-[26px] font-black text-[#4EAAF5]">
                  {num}
                </div>
      ...
      <section className="bg-white py-24 px-6">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-14">
            <p className="text-[12px] font-black tracking-widest text-[#4EAAF5] uppercase mb-3">
              Features
            </p>
            <h2 className="text-[36px] font-black text-gray-900">
              팀 빌딩에 필요한 모든 것
            </h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {FEATURES.map(({ title, desc, icon }) => (
              <div
                key={title}
                className="bg-[#F5F8FD] rounded-2xl p-7 border border-[#E8EFF8]"
              >
      ...
      <section
        ...
        style={{
          background: "linear-gradient(180deg, #E8F3FD 0%, #EEF6FF 100%)",
        }}
      >
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-14">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {STEPS.map(({ step, title, desc }) => (
              <div
                key={step}
                className="bg-white rounded-2xl p-6 border border-[#D8EAF8] shadow-sm"
              >
                <p className="text-[11px] font-black text-[#4EAAF5] tracking-widest mb-3">
                  {step}
                </p>
      ...
      <section
        ...
        style={{
          background: "linear-gradient(135deg, #4EAAF5 0%, #2D8FE0 100%)",
        }}
      >
        <h2 className="text-[34px] font-black text-white mb-3">
          다음 해커톤, 좋은 팀과 함께하세요
        </h2>
        <p className="text-[15px] text-blue-100 mb-10">
          가벼운 1분, 매칭은 AI가 대신해요
        </p>
        <button
          onClick={start}
      ...
      <footer className="bg-white border-t border-[#E8EFF8] px-8 py-5 flex items-center justify-between">
        ...
        <p className="text-[12px] text-gray-400">
          © 2026 Favorite contact. All rights reserved.
        </p>
      </footer>
    </div>
  )
